...
int main(int argc, char **argv) {
  ...
  FILE *csv =
      bench_csv_open(csv_path, "benchmark,n_threads,iterations,is_glibc,time_"
                               "us,throughput_alloc_per_s,peak_rss_kb");
  if (csv) {
    fprintf(csv, "malloc_large,%d,%lld,%d,%.3f,%.3f,%ld\n", nthreads,
            (long long)n_iter, is_glibc, us_per_op, throughput, rss_kb);
    fclose(csv);
  }
